...
def userinfo():
    ...
    for i in range(len(acc_ar)):
        total = acc_ar[i]
        accounts, pwd, qq, count_down = ret(total)
        if "-" in str(count_down):
            print("账号:{},已经过期{}天，请及时续费".format(accounts, count_down))
            print("本次跳过该账号")
            print("=" * 80)
            fail_acc.append("失败，{},已经过期{}天".format(accounts, count_down))
            check = check + 1
            continue
        else:
            print("该账号还有{}天到期哦".format(count_down))
        over = main(accounts, pwd, qq)
        if over == "5":
            check = check + 1
            fail_acc.append("失败: " + accounts + " ,还有{}天到期哦".format(count_down))
            print("学校网站可能崩溃或填报网页api被修改，请联系开发者或者过一段时间重试！！", end="")
            time.sleep(1)
            print("✖")
            print("=" * 80)
        elif over == "1":
            ...
            success_acc.append("成功:" + accounts + "该账号还有{}天到期哦".format(count_down))
            print("=" * 80)
...
def main(accounts, pwd, qq):  # 对单个账号操作
    for i in range(0, 9):
        opt = Options()
        ...
        try:
            ...
            try:
                error = driver.find_element_by_xpath('//*[@id="captcha2"]/div/div[2]/div[3]')
                error.click()
            except:
                time.sleep(2)
                background = driver.find_element(By.XPATH, '//*[@id="captcha2"]/div/div[1]/img[1]').get_attribute(
                    'src')
                img1 = driver.find_element(By.XPATH, '//*[@id="captcha2"]/div/div[1]/img[2]').get_attribute('src')
                img1 = img1.split(',')
                ...
                slide_btn = driver.find_element(By.XPATH, '//*[@id="captcha2"]/div/div[2]/div[2]/div[2]')
                act = ActionChains(driver).drag_and_drop_by_offset(slide_btn, xoffset=result[0] - 72.8, yoffset=0)
                act.perform()
                act.release()
                for u in range(1, 199):  # 设置有99次的重试机会
                    try:
                        time.sleep(2)
                        driver.find_element_by_tag_name('body').send_keys(Keys.CONTROL + Keys.HOME)
                        jktb_web = "my.sues.edu.cn"  # 登录教学服务系统
                        driver.find_element_by_xpath("//*[@id='__layout']/div/div/div[2]/div/div/input").send_keys(
                            str(jktb_web))
                        print("第%s次滑动时成功" % u, end="")
                        time.sleep(0.1)
                        print("✔")
                        break
                    except Exception as e:
                        print("第%s次滑动时失败，正在重试" % u, end="")
                        time.sleep(0.1)
                        print("✖")
                        try:
                            error = driver.find_element_by_xpath('//*[@id="captcha2"]/div/div[2]/div[3]')
                            ...
                            background = driver.find_element(By.XPATH,
                                                             '//*[@id="captcha2"]/div/div[1]/img[1]').get_attribute(
                                'src')
                            img1 = driver.find_element(By.XPATH, '//*[@id="captcha2"]/div/div[1]/img[2]').get_attribute(
                                'src')
                            img1 = img1.split(',')
                            background = background.split(',')
                            i = base64.b64decode(img1[1])
                            ...
                            os.remove('background-%s.png' % pwd)
                            slide_btn = driver.find_element(By.XPATH, '//*[@id="captcha2"]/div/div[2]/div[2]/div[2]')
                            act = ActionChains(driver).drag_and_drop_by_offset(slide_btn, xoffset=result[0] - 72.8,
                                                                               yoffset=0)
                            act.perform()
                            act.release()
            time.sleep(2)
            driver.find_element_by_xpath("//*[@id='__layout']/div/div/div[2]/div/ul/div/div[1]/div/li[1]").click()
            time.sleep(2)
            ...
            button = driver.find_element_by_xpath(
                '/html/body/div[1]/div[2]/div[1]/div[3]/div[2]/div/ul/li[1]/a/p')
            ...
            driver.find_element_by_xpath(
                '//*[@id="form"]/div[10]/div/div/div[2]/div/div/label[2]/div/ins').click()  # 不在校
            driver.find_element_by_xpath(
                '//*[@id="form"]/div[18]/div/div/div[2]/div/div/label[2]').click()  # 健康
            if address1 == address2:
                print("姓名：【{}】,位置：【{}】".format(who, address2 + address + value))
            else:
                print("姓名：【{}】,位置：【{}】".format(who, address1 + address2 + address + value))
            # 输入取值范围，比如100~350
            value = (random.uniform(36, 37))
            # 确定随机小数点位数，比如.3f，表示小数点为3位小数
            value = format(value, '.1f')
            print("自动生成体温：%s℃" % value)  # 将这个随机数打印出来，或者和其他的组成字符串来执行sql语句的操作
            driver.find_element_by_xpath('//*[@id="form"]/div[24]/div/div/div[2]/div/textarea').clear()
            driver.find_element_by_xpath('//*[@id="form"]/div[24]/div/div/div[2]/div/textarea').send_keys(value)
            driver.find_element_by_xpath('//*[@id="post"]').click()
            driver.find_element_by_tag_name('body').send_keys(Keys.CONTROL + Keys.HOME)
            time.sleep(2)
            driver.save_screenshot('log/%s.png' % accounts)
            driver.save_screenshot('C:/qqbot/机器人框架/data/images/%s.png' % accounts)
...
def ret(total):
    ...
    try:
        ...
        if timely == "":
            ...
            timely = re.split(r'[，,]', timely)
            a = datetime.datetime.now()  # 实施时间
            y = str(a.year)
            m = str(a.month)
            d = str(a.day)  # 转换为字符串，便于打印
            # timenow = y + '年' + m + '月' + d + '日' + '\n'
            b = datetime.datetime(int(timely[0]), int(timely[1]), int(timely[2]))  # 自己设置的研究生考试时间
            count_down = (b - a).days  # 考研倒计时
    except Exception as e:
        count_down = "∞"
    return accounts, pwd, qq, count_down
...
def push(accounts, qq, who, address, address1, address2):
    ...
    if accounts == "101319132":
        ...
        try:
            ...
            msg['Subject'] = Header('{}，您好！{}健康填报成功'.format(who, data))
            # 开启发信服务，这里使用的是加密传输
            ...
            server.quit()
            print("本次填报结果已成功推送至：%s@qq.com" % qq, end="")
            time.sleep(0.5)
            print("✔")
        except:
...
def push_plus(title, content):
    ...
    print("pushplus服务已启动，所有账号结果已推送", end="")
    time.sleep(0.5)
    print("✔")
...
def postocr(accounts):
    # API地址
    url = 'http://101.132.172.63:81/'
    # 图片地址
    file_path = "digit-%s.png" % accounts
    # 图片名
...
def postcode(accounts):
    # API地址
    url = 'http://101.132.172.63:81/'
    # 图片地址
    file_path = r'log/%s-schoolcode.png' % accounts
    # 图片名
    file_name = file_path.split('/')[-1]
    # 二进制打开图片
    file = open(file_path, 'rb')
...
if __name__ == '__main__':
    ...
    SUES 体温填报工具 by FY 
    源码/Issue/贡献 https://github.com/666fy666/suestwtb 如果觉得好用别忘记Star噢┗|｀O′|┛！
    ------------------------------------------------------------------------------''')
    try:
        # 读取配置文件并检查
        ...
        try:
            ...
            print("未找到配置文件，启用默认配置！")
            file = {
                "email": True,

                "admin": True,

                "adminemail": "657769008",
