    ...
    for i in range(0, 9):
        ...
        try:
            ...
            try:
                ...
                act = ActionChains(driver).drag_and_drop_by_offset(slide_btn, xoffset=result[0] - 26.8, yoffset=0)
                act.perform()
                act.release()
                for u in range(1, 99):  # 设置有99次的重试机会
                    try:
                        ...
                        try:
                            ...
                            os.remove('img-%s.png' % accounts)
                            os.remove('background-%s.png' % pwd)
                            slide_btn = driver.find_element(By.XPATH,
                                                            '/html/body/div[3]/div/div/div/div[2]/div[2]/div[2]')
                            act = ActionChains(driver).drag_and_drop_by_offset(slide_btn, xoffset=result[0] - 26.8,
                                                                               yoffset=0)
                            act.perform()
                            act.release()
                if email:
